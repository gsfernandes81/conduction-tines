...
class MultiImageEmbedList(list):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for image in images:
            embed = h.Embed(url=url, description="Masked description")
    ...
    def from_embed(
        ...
        multi_image_embed[0].set_image(embed.image)
        multi_image_embed[0].set_footer(embed.footer)
        multi_image_embed[0].set_thumbnail(embed.thumbnail)
        if embed.author:
...
async def pull_from_channel(
    bot: CachedFetchBot,
    after: dt.datetime,
    channel_id: h.Snowflake,
    *,
    merge_content: bool,
    merge_content_into_embed: bool,
    merge_images_into_embed: bool = False,
):
    messages_to_concat: t.List[h.Message] = []
    ...
    async for message in channel.fetch_history(after=after):
        messages_to_concat.append(message)

    attachments = [
        attachment
        for message in messages_to_concat
    ...
    # Concatenate the content, embeds, and attachments if merge_content
    # or merge_content_into_embed is True
    if merge_content or merge_content_into_embed:
        content = "\n".join(
            message.content for message in messages_to_concat if message.content
        )
    if merge_content_into_embed:
        if content:
            if not embeds:
                embeds = [h.Embed(color=embed_default_color)]
            ...
            raise ValueError("Cannot merge content into embed with no content")

    if merge_images_into_embed and len(attachments) > 0:
        embed = embeds.pop()
        logging.error(
            str(getframeinfo(currentframe()).lineno) + " " + embed.description
        )
        # embed = MultiImageEmbed(embed)
        logging.error(
            str(getframeinfo(currentframe()).lineno) + " " + embed[0].description
        )
        for attachment in attachments:
            embed.add_image(attachment.url)
        logging.error(
            str(getframeinfo(currentframe()).lineno) + " " + embed[1].description
        )
        embeds.extend(embed)
        logging.error(
            str(getframeinfo(currentframe()).lineno) + " " + embeds[1].description
        )
        attachments = []
